...
export async function POST() {
  try {
    ...
    // Sync templates from ALL connected WABAs. Each config's WABA
    // is synced independently. Skip configs without a waba_id.
    let totalInserted = 0
    let totalUpdated = 0
    let totalTemplates = 0
    const allErrors: { name: string; language: string; message: string }[] = []
    let anyTruncated = false

    for (const config of configs) {
      if (!config.waba_id) continue
      ...
      for (const t of metaTemplates) {
        ...
        // Lookup scoped to (account_id, waba_id, name, language) to
        // match the new unique index — prevents cross-WABA collisions.
