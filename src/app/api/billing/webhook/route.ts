...
async function syncSubscription(sub: Stripe.Subscription, accountIdOverride?: string): Promise<void> {
  const accountId = accountIdOverride ?? (await resolveAccountId(sub));
  if (!accountId) {
    throw new Error(
    ...
    switch (event.type) {
      case "checkout.session.completed": {
        ...
        if (subId) {
          ...
          await syncSubscription(sub, sub.metadata?.account_id ?? session.metadata?.account_id);
        }
        break;
      }
      ...
      case "invoice.payment_failed": {
        ...
        // The Stripe API includes subscription on the invoice payload
        // but the SDK types model it only inside parent.subscription_details.
        // Use a narrow intersection to read it without an `any` cast.
        const withSub = invoice as Stripe.Invoice & { subscription?: string };
        const subId =
