...
const EDITABLE_ROLES: { value: AccountRole; label: string; hint: string }[] = [
  { value: 'admin', label: 'Admin', hint: 'Gerenciar membros e tudo mais' },
  ...
  async function handleRoleChange(member: Member, nextRole: AccountRole) {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        toast.error(payload.error || 'Falha ao atualizar perfil');
        return;
      }
        ...
        <div>
          ...
          {invitations.length > 0 ? (
            <p className="mb-3 text-xs text-muted-foreground">
              O URL do convite é exibido apenas uma vez por segurança.
              Para compartilhar novamente, revogue o convite abaixo e
              crie um novo.
            </p>
          ) : null}
        ...
        <DialogContent className="bg-popover border-border sm:max-w-sm">
          <DialogHeader>
            ...
            <DialogDescription className="text-muted-foreground">
              ...
              da conta? Ele será desconectado e receberá uma conta pessoal
              nova no próximo login. Seu acesso não será excluído.
            </DialogDescription>
          </DialogHeader>
