...
export function MessageComposer({
  ...
  const stageUpload = useCallback(
    async (kind: ComposerMediaKind, file: File) => {
      ...
      const kindLabels: Record<string, string> = { image: "imagem", video: "vídeo", document: "documento", audio: "áudio" };
      if (file.size > max) {
        toast.error(
          `Arquivo tem ${(file.size / 1024 / 1024).toFixed(1)} MB - limite de ${kindLabels[kind] ?? kind} é ${Math.round(max / 1024 / 1024)} MB.`,
        );
        return;
      }
      setBusy(true);
      ...
      {sessionExpired && (
        <div className="mb-2 flex items-center justify-between rounded-lg bg-amber-500/10 px-3 py-2">
          <p className="text-xs text-amber-400">
            Sessão de 24 horas expirada. Use um modelo para reativar.
          </p>
          <Button
            variant="ghost"
            size="sm"
